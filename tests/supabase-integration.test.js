...
assert(!html.includes('.supabase.co'), 'No Supabase project URL is embedded');
assert(!html.includes('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2'), 'Supabase browser SDK is not loaded');
assert(html.includes('const supabaseClient = null'), 'Supabase client is disabled for local-only mode');
...
assert(html.includes('Saved locally on this browser'), 'App reports local-only persistence');
...
console.log('Local-only persistence static checks passed');
